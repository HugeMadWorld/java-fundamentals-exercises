...
public class CrazyOptionals {
    ...
    public static void deposit(AccountProvider accountProvider, BigDecimal amount) {
        ...
        account.ifPresent(acc -> acc.setBalance(acc.getBalance().add(amount)));
    }
    ...
    public static void processAccount(AccountProvider accountProvider, AccountService accountService) {
        var account = accountProvider.getAccount();
        account.ifPresentOrElse(accountService::processAccount, accountService::processWithNoAccount);
    }
    ...
    public static Optional<Account> retrieveAccountGmail(AccountProvider accountProvider) {
        return accountProvider.getAccount().filter(acc -> acc.getEmail().matches(".*@gmail.com"));
    }
...
//        return accountProvider.getAccount().(fallbackProvider.getAccount().orElseThrow(() -> new NoSuchElementException("")));
        return new Account();
    }
    ...
    public static Account getAccountWithMaxBalance(List<Account> accounts) {
        return accounts.stream().max(Comparator.comparing(Account::getBalance)).orElseThrow(() -> new NoSuchElementException(""));
    }
    ...
    public static OptionalDouble findMinBalanceValue(List<Account> accounts) {
        return OptionalDouble.of(accounts.stream().min(Comparator.comparing(Account::getBalance)).map(account -> account.getBalance().doubleValue()).orElse((double) 0));
    }
    ...
    public static void processAccountWithMaxBalance(List<Account> accounts, AccountService accountService) {
        accountService.processAccount(accounts.stream().max(Comparator.comparing(Account::getBalance)).get());
    }
    ...
    public static double calculateTotalCreditBalance(List<CreditAccount> accounts) {
        return accounts.stream().mapToDouble(cred->cred.getCreditBalance().orElse(BigDecimal.ZERO).doubleValue()).sum();
    }
}
